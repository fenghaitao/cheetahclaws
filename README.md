...
- May 8, 2026 (latest): **NVIDIA NIM free-tier provider + 429 cascade fallback + multi-model `/brainstorm`.** Borrowed selectively from sibling forks (Falcon, Dulus). Adds `nim` provider with curated 10-model free chain (deepseek-r1 / llama-3.3-70b / qwen2.5-coder-32b / …), invoke as `cheetahclaws --model nim/<vendor>/<model>`. On 429 the agent loop swaps to the next NIM model (capped at 3 swaps/turn so a fully-throttled tier doesn't busy-loop). New `/brainstorm --models a,b,c` flag distributes models round-robin across personas — single-model brainstorm is an echo chamber, multi-model buys real epistemic diversity. 21 new pytest cases, zero regressions. Branch: `fix/agentic-on-every-model`.
- May 8, 2026: **"Be agentic on every model" pass — explore-first prompt + qwen overlay + runtime auto-nudge.** Three layered defenses so any model (qwen2.5 / kimi / smaller llamas) explores a directory path instead of asking the user to name a file: `default.md` "Investigate Before Asking" section + softened Stop Conditions, new `prompts/overlays/qwen.md` (cites Qwen function-calling guide), and an `agent.py` one-shot auto-nudge bounded to one retry per user turn. 13 new pytest cases, 89 prompt + 196 targeted regression tests pass, zero regressions. Branch: `fix/agentic-on-every-model`.
